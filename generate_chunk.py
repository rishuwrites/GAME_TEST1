...
def generate_chunk(start_id, end_id, diff, n, seed):
    rng=random.Random(seed)
    levels=[]; seen=set(); t=time.time()
    for lid in range(start_id,end_id+1):
        ...
        levels.append(lv)
        if lid%25==0: print('generated',lid,'elapsed',round(time.time()-t,1),flush=True)
    name=f"levels_{start_id:03d}_{end_id:03d}.json"
    (OUT/name).write_text(json.dumps(levels,separators=(',',':')),encoding='utf8')
    print('WROTE',name,flush=True)

if __name__=='__main__':
    import argparse
    ap=argparse.ArgumentParser(); ap.add_argument('start',type=int); ap.add_argument('end',type=int); ap.add_argument('diff'); ap.add_argument('size',type=int); ap.add_argument('seed',type=int)
    a=ap.parse_args(); generate_chunk(a.start,a.end,a.diff,a.size,a.seed)
